...
static bool MyBeginChildEx(const char* name, ImGuiID id, const char* label, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = ImGui::GetCurrentWindow();
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
...
bool ImGui::GroupBox(const char* str_id, const char* label, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	return MyBeginChildEx(str_id, window->GetID(str_id), label, size_arg, border, extra_flags);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	// Draw frame
	RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2(0, 4), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	...
	if (!is_horizontal)
		grab_t = 1.0f - grab_t;
	const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t); //lazy as fuck
	ImRect grab_bb;
	if (is_horizontal)
		grab_bb = ImRect(ImVec2(grab_pos - grab_sz * 0.5f, frame_bb.Min.y + grab_padding), ImVec2(grab_pos + grab_sz * 0.5f, frame_bb.Max.y - grab_padding));
	else
		grab_bb = ImRect(ImVec2(frame_bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f), ImVec2(frame_bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5f));
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), grab_bb.Max + ImVec2(0, 0) - ImVec2(0, 4), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.25f));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y*0, label_size.y + style.FramePadding.y*0)); // We want a square shape to we use Y twice
    ...
    if (label_size.x > 0.0f)
		RenderText(text_bb.Min - ImVec2(-4, 3.5), label);
...
static bool shutthefuckup(const char* str_id, ImGuiWindowFlags extra_flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	const ImGuiID id = window->GetID(str_id);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f)); // height of the actual combo
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id))
		return false;

	const float arrow_size = (g.FontSize + style.FramePadding.x * 2.0f); // width of the box where the +/- is located at (less = small) (more = wider)
	const bool hovered = ImGui::IsItemHovered();
	bool popup_open = IsPopupOpen(id);
	bool popup_opened_now = false;

	const ImRect value_bb(frame_bb.Min, frame_bb.Max - ImVec2(arrow_size, -4.0f)); // where the +/- is located (-4 max) (less = left)  (more = right)
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDING
	...
	if (*current_item >= 0 && *current_item < items_count)
	{
		...
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(frame_bb.Min + ImVec2(3.5, 3), value_bb.Max, item_text, NULL, NULL, ImVec2(0.0f, 0.0f));
	}

	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + 3, frame_bb.Min.y - 0), label);
	...
	if (!IsPopupOpen(id))
	{
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - (arrow_size / 2) - 0.5, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x - (arrow_size / 2) - 0.5, frame_bb.Max.y - 4), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 5, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Max.x - 5, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
	}
	if (IsPopupOpen(id))
	{
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 5, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Max.x - 5, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
		...
		const ImGuiWindowFlags flags = ImGuiWindowFlags_ComboBox | ((window->Flags));
		if (shutthefuckup(label, flags))
		{
			// Display items
			Spacing();
